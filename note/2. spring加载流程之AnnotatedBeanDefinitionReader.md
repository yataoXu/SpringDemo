...
- DefaultListableBeanFactory的用途:spring加载的bean都会放到这里面去，上面的MainConfig就是在DefaultListableBeanFactory里面。
- GenericApplicationContext的父类AbstractApplicationContext，包括AbstractApplicationContext的父类DefaultResourceLoader都有默认构造函数
...
```
1. 首先第一个构造函数里面的参数registry其实就是当前上下文AnnotationConfigApplicationContext，因为传的是this就是自己，
从这里也可以看出AnnotationConfigApplicationContext实现了BeanDefinitionRegistry接口
...
除了jpa的那个bean(beanName为org.springframework.context.annotation.internalPersistenceAnnotationProcessor)没有之外，
